...
namespace Asm_UWP.Model
{
    public class ApiManager
    {
        public static async Task<List<RootObject>> GetArticle()
        {
            var http = new HttpClient();
            var url =
                "http://tommyprivateguide.com/wp-json/wp/v2/posts?param%2Fdata.json&fbclid=IwAR2LzEhbbgd6n2G2mDJY-N-iJ7CiJk5RgJ-6FGHtXLQWFko0k4hBmypPGJY";
            var resp = await http.GetAsync(url);
            var result = await resp.Content.ReadAsStringAsync();
            List<RootObject> list = JsonConvert.DeserializeObject<List<RootObject>>(result);
            return list;
        }
